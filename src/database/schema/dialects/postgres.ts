...
export default class Postgres implements SchemaInspector {
	...
	async overview(): Promise<SchemaOverview> {
		...
		const [columnsResult, primaryKeysResult] = await Promise.all([
			// Views are supported, but you need to make sure each view had an id field (which will be set as PK automatically)
			// Views are read only, so they cannot be used for anything other than read operations
			this.knex.raw(
				`
