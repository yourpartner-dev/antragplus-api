...
import getModuleDefault from '../../helpers/utils/get-module-default.js';
...
export default async function run(database: Knex, direction: 'up' | 'down' | 'latest', log = true): Promise<void> {
	...
	migrationFiles = migrationFiles.filter((file: string) => 
		/^[0-9]+[A-Z]-[^.]+\.(?:js|ts)$/.test(file) && !file.endsWith('.d.ts')
	);

	customMigrationFiles = customMigrationFiles.filter((file: string) => file.includes('-') && /\.(c|m)?js$/.test(file));

	const completedMigrations = await database.select<Migration[]>('*').from('yp_migrations').orderBy('version');
	...
	async function up() {
		...
		await up(database);
		await database.insert({ version: nextVersion.version, name: nextVersion.name }).into('yp_migrations');
	...
	async function down() {
		...
		await down(database);
		await database('yp_migrations').delete().where({ version: migration.version });
	...
	async function latest() {
		...
		for (const migration of migrations) {
			if (migration.completed === false) {
				...
				await up(database);
				await database.insert({ version: migration.version, name: migration.name }).into('yp_migrations');
			}
		}
