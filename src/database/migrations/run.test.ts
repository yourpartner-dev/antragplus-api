...
describe('run', () => {
	...
	describe('when passed the argument up', () => {
		it('returns "Nothing To Upgrade" if no yp_migrations', async () => {
			tracker.on.select('yp_migrations').response(['Empty']);
		...
		it('returns "Method implemented in the dialect driver" if no yp_migrations', async () => {
			tracker.on.select('yp_migrations').response([]);
		...
		it('returns undefined if the migration is successful', async () => {
			...
			tracker.on.insert('yp_migrations').response(['Remove System Relations', '20201029A']);
	...
	describe('when passed the argument down', () => {
		it('returns "Nothing To downgrade" if no valid yp_migrations', async () => {
			tracker.on.select('yp_migrations').response(['Empty']);
