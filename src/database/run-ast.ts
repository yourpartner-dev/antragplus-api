...
import type { Item, Query, SchemaOverview } from '../types/index.js';
...
import { applyFunctionToColumnName } from '../helpers/utils/apply-function-to-column-name.js';
import applyQuery, { applyLimit, applySort, generateAlias, type ColumnSortRecord } from '../helpers/utils/apply-query.js';
import { getCollectionFromAlias } from '../helpers/utils/get-collection-from-alias.js';
import type { AliasMap } from '../helpers/utils/get-column-path.js';
import { getColumn } from '../helpers/utils/get-column.js';
import { parseFilterKey } from '../helpers/utils/parse-filter-key.js';
...
async function getDBQuery(
	...
	if (sortRecords) {
		...
		if (needsInnerQuery) {
			...
			if (hasMultiRelationalSort) {
				...
				// Start order by with yp_row_number. The yp_row_number is derived from a window function that
				...
				// Since the only relevant result is the first row of this partition, adding the yp_row_number to the
				// order by here ensures that all rows with a yp_row_number = 1 show up first in the inner query result,
				// and are correctly truncated by the limit, but not earlier.
				orderByString = `?? asc, ${orderByString}`;
				orderByFields.unshift(knex.ref('yp_row_number'));
			}
	...
	if (sortRecords) {
		...
		if (hasMultiRelationalSort) {
			wrapperQuery.where('inner.yp_row_number', '=', 1);
