import { ErrorCode, InternalServerError, isYPError } from '../helpers/errors/index.js';
import type { DeepPartial } from '../types/index.js';
import { isObject } from '../helpers/utils/index.js';
import { getNodeEnv } from '../helpers/utils/node/index.js';
...
export const errorHandler = asyncErrorHandler(async (err, req, res) => {
	...
	for (const error of receivedErrors) {
		...
		if (isYPError(error)) {
			...
			if (isYPError(error, ErrorCode.MethodNotAllowed)) {
