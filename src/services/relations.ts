...
import type { ForeignKey, SchemaInspector } from '../database/schema/index.js';
import { createInspector } from '../database/schema/index.js';
import { systemRelationRows } from '../helpers/system-data/index.js';
import type { Accountability, Query, Relation, RelationMeta, SchemaOverview } from '../types/index.js';
...
import { getDefaultIndexName } from '../helpers/utils/get-default-index-name.js';
...
export class RelationsService {
	...
	async readAll(collection?: string, _opts?: QueryOptions): Promise<Relation[]> {
		...
		const results = this.stitchRelations(schemaRows, metaRows,);
	...
	async readOne(collection: string, field: string): Promise<Relation> {
		...
		const stitched = this.stitchRelations(schemaRow ? [schemaRow] : []);
	...
	/**
	 * Combine raw schema foreign key information with to form final
	 * Relation objects
	 */
	private stitchRelations(schemaRows: ForeignKey[], metaRows?: RelationMeta[]) {
