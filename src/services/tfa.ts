...
export class TFAService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.itemsService = new ItemsService('yp_users', options);
	}
	...
	async verifyOTP(key: PrimaryKey, otp: string, secret?: string): Promise<boolean> {
		...
		const user = await this.knex.select('tfa_secret').from('yp_users').where({ id: key }).first();
	...
	async generateTFA(key: PrimaryKey): Promise<Record<string, string>> {
		const user = await this.knex.select('email', 'tfa_secret').from('yp_users').where({ id: key }).first();
		...
		return {
			secret,
			url: authenticator.keyuri(user.email, 'YourPartner', secret),
		};
	}
