...
import { ContentTooLargeError, ForbiddenError, InvalidPayloadError, ServiceUnavailableError } from '../helpers/errors/index.js';
import formatTitle from '../helpers/format-title/index.js';
import type { BusboyFileStream, File, PrimaryKey, Query } from '../types/index.js';
...
import { getAxios } from '../helpers/request/index.js';
...
export class FilesService extends ItemsService<File> {
	...
	async uploadOne(
		...
		const sudoService = new ItemsService('yp_files', {
			knex: this.knex,
			schema: this.schema,
		});
	...
	async importOne(importURL: string, body: Partial<File>): Promise<PrimaryKey> {
		const fileCreatePermissions = this.accountability?.permissions?.find(
			(permission) => permission.collection === 'yp_files' && permission.action === 'create',
		);
