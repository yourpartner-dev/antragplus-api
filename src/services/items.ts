...
import { ErrorCode, ForbiddenError, InvalidPayloadError, isYPError } from '../helpers/errors/index.js';
...
import getASTFromQuery from '../helpers/utils/get-ast-from-query.js';
import { shouldClearCache } from '../helpers/utils/should-clear-cache.js';
import { transaction } from '../helpers/utils/transaction.js';
import { validateKeys } from '../helpers/utils/validate-keys.js';
...
export class ItemsService<Item extends AnyItem = AnyItem, Collection extends string = string>
	...
	constructor(collection: Collection, options: AbstractServiceOptions) {
		...
		this.eventScope = isSystemCollection(this.collection) ? this.collection.substring(3) : 'items';
	...
	async createOne(data: Partial<Item>, opts: MutationOptions = {}): Promise<PrimaryKey> {
		...
		const primaryKey: PrimaryKey = await transaction(this.knex, async (trx) => {
			...
			try {
				...
				if (isYPError(dbError, ErrorCode.RecordNotUnique) && dbError.extensions.primaryKey) {
			...
			//TODO: what about system created items in QUEUE?
	...
	async deleteMany(keys: PrimaryKey[], opts: MutationOptions = {}): Promise<PrimaryKey[]> {
		...
		// Fetch the complete objects before deletion
		//TODO: this is only for the dearest_shares hook for now, or other hooks that need the complete object.
		// We should find a better way to do this.
		const deletedObjects = await this.readMany(keys);
