import { randomIdentifier, randomInteger, randomUUID } from '../../helpers/random/index.js';
import type {
	Accountability,
	CollectionsOverview,
	DeepPartial,
...
const ypPermissionsSchema: DeepPartial<CollectionsOverview[string]> = {
	primary: 'id',
	fields: {
...
describe('Services / PermissionsService', () => {
	describe('with app minimal permissions', async () => {
		...
		beforeEach(() => {
			...
			const schema: DeepPartial<SchemaOverview> = {
				collections: {
					yp_permissions: ypPermissionsSchema,
				},
				relations: [],
			};
			...
			tracker.on.select('select "yp_permissions"').response(sample.permissions);
	...
	describe('#getItemPermissions', () => {
		...
		describe('singleton', () => {
			...
			const permissionReadAccess = { ...permissionPreset, collection: 'yp_permissions', action: 'read' };
			...
			test('use create permission if singleton does not exist', async () => {
				...
				tracker.on.select('yp_permissions').response(permissions);
			...
			test('requires permissions on yp_permissions to return presets and fields', async () => {
