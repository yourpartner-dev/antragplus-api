...
import type { Range, Stat } from '../storage/storage-manager/index.js';
import type { Accountability, File } from '../types/index.js';
...
import * as TransformationUtils from '../helpers/utils/transformations.js';
...
export class AssetsService {
	...
	async getAsset(
		 ...
		 * with a wrong type. In case of yp_files where id is a uuid, we'll have to verify the
		 * validity of the uuid ahead of time.
		 */
		if (!isValidUuid(id)) throw new ForbiddenError();

		if (systemPublicKeys.includes(id) === false && this.accountability?.admin !== true) {
			await this.authorizationService.checkAccess('read', 'yp_files', id);
		}
