...
import type { AbstractServiceOptions, Accountability, ActionEventParams, File, Query, SchemaOverview } from '../types/index.js';
...
import { ForbiddenError, InvalidPayloadError, ServiceUnavailableError, UnsupportedMediaTypeError } from '../helpers/errors/index.js';
import { transaction } from '../helpers/utils/transaction.js';
import { getService } from '../helpers/utils/get-service.js';
import { createTmpFile } from '../helpers/utils/node/index.js';
import { parseJSON } from '../helpers/utils/parse-json.js';
import { getDateFormatted } from '../helpers/utils/get-date-formatted.js';
import { toArray } from '../helpers/utils/to-array.js';
import { Url } from '../helpers/utils/url.js';
import { userName } from '../helpers/utils/user-name.js';
...
export class ExportService {
	...
	async exportToFile(
		...
		const { createTmpFile } = await import('../helpers/utils/node/index.js');
		const tmpFile = await createTmpFile().catch(() => null);
		...
		try {
			...
			if (this.accountability?.user) {
				...
				const message = `Hello ${userName(user)}, Your export of ${collection} is ready. <a href="${href}">Click here to view.</a>`;
