...
import formatTitle from '../helpers/format-title/index.js';
import type { Accountability, FieldOverview, Permission, SchemaOverview, Type } from '../types/index.js';
...
import { getRelationType } from '../helpers/utils/get-relation-type.js';
...
import { loadYamlFile } from '../helpers/system-data/load-yaml.js';
...
const spec: OpenAPIObject = loadYamlFile(join(__dirname, '../specs/openapi.yaml')) as OpenAPIObject;
...
class OASSpecsService implements SpecificationSubService {
	...
	async generate(host?: string) {
		...
		const spec: OpenAPIObject = {
			openapi: '3.0.1',
			info: {
				title: 'Real time API Specification',
				description:
			...
			servers: [
				{
					url,
					description: 'Your current yourpartner instance.',
				},
			],
			paths,
		};
	...
	private async generateComponents(tags: OpenAPIObject['tags']): Promise<OpenAPIObject['components']> {
		...
		for (const [name, schema] of Object.entries(spec.components?.schemas ?? {})) {
			if (requiredSchemas.includes(name)) {
				const collection = spec.tags?.find((tag:any) => tag.name === name)?.['x-collection'];
