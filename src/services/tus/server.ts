...
import type { Driver, TusDriver} from '../../storage/storage-manager/index.js';
import { supportsTus} from '../../storage/storage-manager/index.js';
import type { Accountability, File, SchemaOverview } from '../../types/index.js';
import { toArray } from '../../helpers/utils/index.js';
...
export async function createTusServer(context: Context): Promise<[Server, () => void]> {
	...
	const server = new Server({
		...
		async onUploadFinish(req: any, res, upload) {
			const service = new ItemsService<File>('yp_files', {
				schema: req.schema,
			});
	...
	server.on(EVENTS.POST_CREATE, async (_req, res, upload) => {
		res.setHeader('YP-File-Id', upload.metadata!['id']!);
	});
