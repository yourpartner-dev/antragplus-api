import { Action, FUNCTIONS } from '../../constants.js';
import { useEnv } from '../../helpers/env/index.js';
import { ErrorCode, ForbiddenError, InvalidPayloadError, isYPError, type YPError } from '../../helpers/errors/index.js';
import { isSystemCollection } from '../../helpers/system-data/index.js';
import type { Accountability, Aggregate, Filter, Item, PrimaryKey, Query, SchemaOverview } from '../../types/index.js';
import { parseFilterFunctionPath, toBoolean } from '../../helpers/utils/index.js';
...
import { getGraphQLType } from '../../helpers/utils/get-graphql-type.js';
...
import { reduceSchema } from '../../helpers/utils/reduce-schema.js';
import { sanitizeQuery } from '../../helpers/utils/sanitize-query.js';
import { validateQuery } from '../../helpers/utils/validate-query.js';
...
const READ_ONLY = ['yp_activity', 'yp_revisions'];
...
export class GraphQLService {
		...
		if (readableCollections.length > 0) {
			schemaComposer.Query.addFields(
				readableCollections.reduce(
					(acc, collection) => {
						const collectionName = this.scope === 'items' ? collection.collection : collection.collection.substring(3);
					...
					.reduce(
						(acc, collection) => {
							const collectionName =
								this.scope === 'items' ? collection.collection : collection.collection.substring(3);
					...
					fields: Object.values(collection.fields).reduce(
						(acc, field) => {
							...
							if (collection.primary === field.field) {
								...
								if (collection.collection === 'yp_permissions') {
									type = GraphQLID;
	 ...
	 * YourPartners' query structure which is then executed by the services.
	 */
	async resolveQuery(info: GraphQLResolveInfo): Promise<Partial<Item> | null> {
		let collection = info.fieldName;
		if (this.scope === 'system') collection = `yp_${collection}`;
	 ...
	 * whole nested tree into one big query using YourPartners' own query resolver, we want to have a nested structure of
	 * arguments for the whole resolving tree, which can later be transformed into YourPartners' AST using `deep`.
	...
	/**
	 * Get a YourPartners Query object from the parsed arguments (rawQuery) and GraphQL AST selectionSet. Converts SelectionSet into
	 * YourPartners' `fields` query for use in the resolver. Also applies variables where appropriate.
	 */
	getQuery(
		rawQuery: Query,
	...
	/**
	 * Replace functions from GraphQL format to YourPartner-Filter format
	 */
	...
	/**
	 * Convert YourPartner-Exception into a GraphQL format, so it can be returned by GraphQL properly.
	 */
	formatError(error: YPError | YPError[]): GraphQLError {
	...
	injectSystemResolvers(
				...
				project: {
					type: new GraphQLObjectType({
						...
						fields: {
							...
							public_registration_role: { type: GraphQLString },
							public_registration_email_filter: { type: GraphQLJSON },
							public_favicon: { type: GraphQLString },
						},
					}),
				},
			},
		...
		schemaComposer.Mutation.addFields({
			...
			users_invite_accept: {
				...
				resolve: async (_, args) => {
					...
					await service.acceptInvite(args['token'], args['password'], args['email']);
					return true;
				},
			},
			users_register: {
		...
		if ('yp_users' in schema.read.collections) {
			schemaComposer.Query.addFields({
				users_me: {
					type: ReadCollectionTypes['yp_users']!,
		...
		if ('yp_users' in schema.update.collections && this.accountability?.user) {
			schemaComposer.Mutation.addFields({
				update_users_me: {
					type: ReadCollectionTypes['yp_users']!,
					args: {
						data: toInputObjectType(UpdateCollectionTypes['yp_users']!),
					},
					resolve: async (_, args, __, info) => {
						if (!this.accountability?.user) return null;
						...
						if ('yp_users' in ReadCollectionTypes) {
							const selections = this.replaceFragmentsInSelections(
								info.fieldNodes[0]?.selectionSet?.selections,
								info.fragments,
							);
		...
		if ('yp_activity' in schema.create.collections) {
			schemaComposer.Mutation.addFields({
				create_comment: {
					type: ReadCollectionTypes['yp_activity'] ?? GraphQLBoolean,
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						item: new GraphQLNonNull(GraphQLID),
					...
					resolve: async (_, args, __, info) => {
						...
						if ('yp_activity' in ReadCollectionTypes) {
							const selections = this.replaceFragmentsInSelections(
								info.fieldNodes[0]?.selectionSet?.selections,
								info.fragments,
							);
		...
		if ('yp_activity' in schema.update.collections) {
			schemaComposer.Mutation.addFields({
				update_comment: {
					type: ReadCollectionTypes['yp_activity'] ?? GraphQLBoolean,
					args: {
		...
		if ('yp_activity' in schema.delete.collections) {
			schemaComposer.Mutation.addFields({
				delete_comment: {
					type: DeleteCollectionTypes['one']!,
					args: {
		...
		if ('yp_files' in schema.create.collections) {
			schemaComposer.Mutation.addFields({
				import_file: {
					type: ReadCollectionTypes['yp_files'] ?? GraphQLBoolean,
					args: {
						url: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(CreateCollectionTypes['yp_files']!).setTypeName(
							'create_yp_files_input',
						),
					},
					resolve: async (_, args, __, info) => {
						...
						if ('yp_files' in ReadCollectionTypes) {
							const selections = this.replaceFragmentsInSelections(
								info.fieldNodes[0]?.selectionSet?.selections,
								info.fragments,
							);
		...
		if ('yp_users' in schema.create.collections) {
			schemaComposer.Mutation.addFields({
				users_invite: {
					type: GraphQLBoolean,
					args: {
					...
					resolve: async (_, args) => {
						...
						await service.inviteUser(args['email'], args['role'], args['invite_url'] || null, params);
						return true;
					},
				},
			});
