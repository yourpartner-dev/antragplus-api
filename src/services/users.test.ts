import { ForbiddenError, InvalidPayloadError, RecordNotUniqueError } from '../helpers/errors/index.js';
...
describe('Integration Tests', () => {
	...
	beforeEach(() => {
		tracker.on.any('yp_users').response({});

		// mock notifications update query in deleteOne/deleteMany/deleteByQuery methods
		tracker.on.update('yp_notifications').response({});

		// mock user counts in updateOne/updateMany/updateByQuery methods
		tracker.on
			.select(
				/(select count\("yp_users"\."id"\) as "count", "yp_roles"\."admin_access", "yp_roles"\."app_access" from "yp_users").*(group by "yp_roles"\."admin_access", "yp_roles"\."app_access")/,
			)
	...
	describe('Services / Users', () => {
		...
		describe('updateOne', () => {
			it('should not checkRemainingAdminExistence', async () => {
				...
				tracker.on
					.select(/select "admin_access", "app_access" from "yp_roles"/)
		...
		describe('updateMany', () => {
			...
			it('should throw RecordNotUniqueError for multiple keys with same email', async () => {
				...
				try {
					...
					expect(err.message).toBe(`Value for field "email" in collection "yp_users" has to be unique.`);
		...
		//@TODO: Make tests work as invitUser function is changed
		describe('invite', () => {
			it('should invite new users', async () => {
				// mock newRole query in updateMany
				tracker.on
					.select(/select "id", "admin_access", "app_access" from "yp_roles"/)
