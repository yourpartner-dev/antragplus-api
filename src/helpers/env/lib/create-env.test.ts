...
import { isYourPartnerVariable } from '../utils/is-yourpartner-variable.js';
...
vi.mock('../utils/is-yourpartner-variable.js');
...
test('Reads value from file if key is a file key', () => {
	...
	vi.mocked(isYourPartnerVariable).mockImplementation((key) => {
		return key === 'TEST_FILE';
	});
...
test('Passthrough file variables that are not YourPartner configuration flags', () => {
	vi.mocked(readConfigurationFromFile).mockReturnValue({
		TEST_FILE: './test/path',
	});

	vi.mocked(isYourPartnerVariable).mockImplementation(() => {
		return false;
	});
