...
import { InvalidQueryError } from '../../helpers/errors/index.js';
...
/*
{
  ...
  collection: 'articles',
  aliasMap: {
    author: { alias: 'grenv', collection: 'yp_users' },
	'author.role': { alias: 'ljnsv', collection: 'yp_roles' },
  },
  relations: []
...
test('Returns correct alias for m2o', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		collection: 'articles',
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'yp_users' },
			'author.role': { alias: 'grenv', collection: 'yp_roles' },
		},
		relations: [
			{
				collection: 'articles',
				field: 'author',
				related_collection: 'yp_users',
				meta: null,
				schema: null,
			},
			{
	...
	expect(result.targetCollection).toBe('yp_roles');
});
...
test('Returns correct alias when there are multiple joins to the same table', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'yp_users' },
			'author.role': { alias: 'grenv', collection: 'yp_roles' },
			'author.secondary_role': { alias: 'psgwn', collection: 'yp_roles' },
		},
		relations: [
			{
				collection: 'articles',
