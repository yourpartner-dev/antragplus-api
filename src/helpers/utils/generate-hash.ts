...
export function generateHash(stringToHash: string): Promise<string> {
	const argon2HashConfigOptions = getConfigFromEnv('HASH_', 'HASH_RAW'); // Disallow the HASH_RAW option 

	// associatedData, if specified, must be passed as a Buffer to argon2.hash
