...
import { TokenExpiredError, InvalidTokenError } from '../../helpers/errors/index.js';
import { ServiceUnavailableError } from '../../helpers/errors/index.js';

const payload: YourPartnerTokenPayload = { role: null, app_access: false, admin_access: false };
const secret = 'test-secret';
const options = { issuer: 'yourpartner' };
...
test('Returns the payload of a correctly signed token', () => {
	...
	expect(result['iss']).toBe('yourpartner');
...
const RequiredEntries: Array<keyof YourPartnerTokenPayload> = ['role', 'app_access', 'admin_access'];
